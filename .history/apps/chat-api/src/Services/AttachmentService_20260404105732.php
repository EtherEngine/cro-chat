...
final class AttachmentService
{
    ...
    public static function store(array $file, int $messageId): array
    {
        ...
        if (($file['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
            throw ApiException::validation('Upload fehlgeschlagen (error ' . ($file['error'] ?? 'unknown') . ')', 'UPLOAD_FAILED');
        }

        $tmpPath = $file['tmp_name'] ?? '';
        if (!is_uploaded_file($tmpPath)) {
            throw ApiException::validation('Ungültiger Upload', 'UPLOAD_INVALID');
        }
        ...
        if ($fileSize > $maxSize || $fileSize === 0) {
            throw ApiException::validation('Datei zu groß (max. ' . ($maxSize / 1024 / 1024) . ' MB)', 'UPLOAD_TOO_LARGE');
        }
        ...
        if ($mimeType === false || !in_array($mimeType, self::ALLOWED_TYPES, true)) {
            throw ApiException::validation('Dateityp nicht erlaubt: ' . ($mimeType ?: 'unbekannt'), 'UPLOAD_TYPE_DENIED');
        }
        ...
        if (!move_uploaded_file($tmpPath, $destPath)) {
            throw ApiException::internal('Speichern fehlgeschlagen');
        }
    ...
    public static function download(int $attachmentId, int $userId): never
    {
        ...
        if (!$attachment) {
            throw ApiException::notFound('Anhang nicht gefunden', 'ATTACHMENT_NOT_FOUND');
        }
        ...
        if ($realPath === false || !str_starts_with($realPath, $realUploadDir)) {
            throw ApiException::notFound('Datei nicht gefunden', 'FILE_NOT_FOUND');
        }
    ...
    private static function requireMessageAccess(array $message, int $userId): void
    {
        ...
        throw ApiException::forbidden('Kein Zugriff', 'ATTACHMENT_ACCESS_DENIED');
    }

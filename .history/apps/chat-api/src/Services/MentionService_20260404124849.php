...
final class MentionService
{
     ...
     * Supports display names with letters, digits, spaces, dots, hyphens, underscores.
     * Uses a greedy approach: tries longest match first against known names.
     */
    public static function parseMentions(string $body): array
    {
        // Match @ followed by one or more word chars (possibly with dots/hyphens)
        // We'll capture the raw token after @ and resolve against actual display names
        if (!preg_match_all('/(?:^|(?<=\s))@([\w][\w .\\-]{0,98}[\w]|[\w])/u', $body, $matches)) {
            return [];
        }

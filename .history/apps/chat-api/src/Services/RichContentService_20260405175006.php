...
final class RichContentService
{
    ...
    private const SUPPORTED_LANGUAGES = [
        'text','javascript','typescript','php','python','java','csharp','cpp',
        'go','rust','ruby','swift','kotlin','sql','html','css','scss',
        'json','yaml','xml','markdown','bash','powershell','dockerfile',
    ];
    ...
    public static function createSnippet(int $spaceId, int $userId, array $input): array
    {
        ...
        $snippet = RichContentRepository::createSnippet(
            $spaceId, $userId, $title, $content, $language, $description, $channelId, (bool) $isPublic
        );
    ...
    private static function fetchUrlMetadata(string $url): array
    {
        ...
        if ($metadata['title']) $metadata['title'] = mb_substr($metadata['title'], 0, 500);
        if ($metadata['description']) $metadata['description'] = mb_substr($metadata['description'], 0, 1000);
        if ($metadata['site_name']) $metadata['site_name'] = mb_substr($metadata['site_name'], 0, 200);
    ...
    public static function createDraft(int $spaceId, int $userId, array $input): array
    {
        ...
        $draft = RichContentRepository::createDraft(
            $spaceId, $userId, $body, $title, $format, $channelId, $conversationId
        );

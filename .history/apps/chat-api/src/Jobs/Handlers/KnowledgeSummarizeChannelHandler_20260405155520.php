    ...
    private function summarizeCluster(array $cluster): ?string
    {
        ...
        foreach ($cluster as $msg) {
            ...
            if (preg_match('/\?/', $msg['body']))
                $score += 2;
            if (preg_match('/\b(entschieden|beschlossen|fazit|solution|ergebnis)\b/iu', $msg['body']))
                $score += 8;
            if ($score > $bestScore) {
                $bestScore = $score;
                $best = $msg;

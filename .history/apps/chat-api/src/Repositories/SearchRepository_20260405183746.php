...
final class SearchRepository
{
    ...
    /**
     * Build highlight ranges: array of [offset, length, term] for each match,
     * so the frontend can render <mark> tags.
     */
    private static function buildHighlights(string $body, array $terms): array
    {
        $highlights = [];
        ...
        foreach ($terms as $term) {
            ...
            while (($pos = mb_strpos($lowerBody, $lowerTerm, $searchFrom)) !== false) {
                $highlights[] = [
                    'offset' => $pos,
                    'length' => $termLen,
                    'term'   => mb_substr($body, $pos, $termLen),
                ];
                $searchFrom = $pos + $termLen;
            }
        }

        // Sort by offset
        usort($highlights, fn($a, $b) => $a['offset'] - $b['offset']);

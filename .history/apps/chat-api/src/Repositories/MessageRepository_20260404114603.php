...
final class MessageRepository
{
     ...
     */
    private static function hydrate(array $row, array $attachmentMap = []): array
    {
        $id = (int) $row['id'];
        $msg = [
            'id' => $id,
    ...
    private static function hydrateMany(array $rows): array
    {
        ...
        return array_map(fn($r) => self::hydrate($r, $attachmentMap), $rows);
    }
    ...
    public static function find(int $id): ?array
    {
        ...
        $row = $stmt->fetch();
        if (!$row) return null;
        $attachmentMap = self::attachmentMapFor([(int) $row['id']]);
        return self::hydrate($row, $attachmentMap);
    }
    ...
    private static function cursorQuery(
        ...
        $messages = array_map([self::class, 'hydrate'], $rows);
    ...
    public static function create(
        ...
        $stmt = $db->prepare(
            'INSERT INTO messages (body, user_id, channel_id, conversation_id, reply_to_id, idempotency_key)
             VALUES (?, ?, ?, ?, ?, ?)'
        );
        $stmt->execute([$body, $userId, $channelId, $conversationId, $replyToId, $idempotencyKey]);

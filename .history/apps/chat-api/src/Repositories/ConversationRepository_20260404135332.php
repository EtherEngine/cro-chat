...
final class ConversationRepository
{
     ...
     * For group: always creates a new conversation (hash is NULL so no dedup).
     */
    public static function getOrCreate(
        ...
        // 1:1 DMs use participant_hash for dedup; groups don't
        $hash = $isGroup ? null : self::participantHash($userIds);

        if (!$isGroup && $hash !== null) {
        ...
        try {
            $stmt = $db->prepare(
                'INSERT INTO conversations (space_id, is_group, title, created_by, avatar_url, participant_hash)
                 VALUES (?, ?, ?, ?, ?, ?)'
            );
            $stmt->execute([$spaceId, $isGroup ? 1 : 0, $title, $createdBy, $avatarUrl, $hash]);
            ...
            $db->rollBack();
            // Duplicate key → race condition on 1:1, another request won
            if (!$isGroup && (int) $e->errorInfo[1] === 1062 && $hash !== null) {
                $existing = self::findByHash($spaceId, $hash);
                if ($existing) {
                    $existing['users'] = self::members((int) $existing['id']);
                    return $existing;
    ...
    public static function updateAvatarUrl(int $id, string $avatarUrl): void
    {
        Database::connection()->prepare(
            'UPDATE conversations SET avatar_url = ? WHERE id = ?'
        )->execute([$avatarUrl, $id]);
    }

    public static function addMember(int $conversationId, int $userId): bool
    {
        try {
            Database::connection()->prepare(
                'INSERT INTO conversation_members (conversation_id, user_id) VALUES (?, ?)'
    ...
    public static function removeMember(int $conversationId, int $userId): bool
    {
        $stmt = Database::connection()->prepare(

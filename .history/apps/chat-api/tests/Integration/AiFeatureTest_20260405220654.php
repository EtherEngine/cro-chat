...
final class AiFeatureTest extends TestCase
{
    ...
    {
        $this->actingAs($this->admin['id']);
        $this->assertApiException(422, 'INVALID_PROVIDER', function () {
    ...
    public function test_thread_summary_empty_thread_fails(): void
    {
        ...
        $this->assertApiException(422, 'NO_MESSAGES', function () use ($thread) {
    ...
    {
        $this->actingAs($this->user['id']);
        $this->assertApiException(422, 'NO_MESSAGES', function () {
    ...
    public function test_update_action_item_invalid_status(): void
    {
        ...
        $this->assertApiException(422, 'INVALID_STATUS', function () use ($item) {
    ...
    {
        $this->actingAs($this->user['id']);
        $m1 = $this->createMessage($this->admin['id'], $this->channel['id'], null, 'PHP Programmierung Webentwicklung Framework Backend Server');
        $m2 = $this->createMessage($this->user['id'], $this->channel['id'], null, 'TypeScript Frontend React Komponenten Design CSS');
        $m3 = $this->createMessage($this->admin['id'], $this->channel['id'], null, 'MariaDB Datenbank SQL Query Index Performance');
        ...
        $results = AiService::semanticSearch($this->space['id'], $this->user['id'], 'PHP Programmierung Backend');
    ...
    public function test_accept_suggestion_invalid_index(): void
    {
        ...
        $this->assertApiException(422, 'INVALID_INDEX', function () use ($suggestion) {
    ...
    {
        $this->actingAs($this->user['id']);
        $this->assertApiException(422, 'INVALID_SCOPE', function () {
    ...
    public function test_ai_summarize_channel_handler(): void
    {
        ...
        // Use today's date range so messages are found
        $start = date('Y-m-d 00:00:00');
        $end = date('Y-m-d 23:59:59');
        $summary = AiService::summarizeChannel($this->channel['id'], $this->space['id'], $this->admin['id'], $start, $end);

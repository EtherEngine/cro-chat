...
final class ScalingTest extends TestCase
{
    ...
    {
        ObjectStorage::putContent('Hello World', 'test/hello.txt', 'text/plain');
        $content = ObjectStorage::get('test/hello.txt');
    ...
    {
        ObjectStorage::putContent('content', 'pub/file.txt', 'text/plain');
        $url = ObjectStorage::url('pub/file.txt');
        $this->assertStringContainsString('pub/file.txt', $url);
    }

    public function test_storage_info(): void
    {
        ObjectStorage::putContent('test', 'info.txt', 'text/plain');
        $info = ObjectStorage::info('info.txt');
        $this->assertSame(4, $info['size']);
        $this->assertArrayHasKey('modified', $info);
    }

    public function test_storage_info_returns_null_for_missing(): void
    {
        $this->assertNull(ObjectStorage::info('missing.txt'));
    }

    public function test_storage_put_creates_subdirectories(): void
    {
        ObjectStorage::putContent('deep', 'deep/nested/dir/file.txt', 'text/plain');
        $this->assertSame('deep', ObjectStorage::get('deep/nested/dir/file.txt'));
    }

    public function test_storage_overall_info(): void
    {
        $info = ObjectStorage::getInfo();
    ...
    public function test_cache_user_pattern(): void
    {
        ...
        ScalingService::cacheUser($user['id']);

...
$heather = UserRepository::findByEmail('heather.mason@cro.dev');
if (!$heather) {
    logLine("FATAL: User heather.mason@cro.dev nicht gefunden.");
    exit(1);
}
$heatherId = (int) $heather['id'];
logLine("Sender: {$heather['display_name']} (ID {$heatherId})");
...
while (!$shouldStop) {
    ...
    try {
        ...
        // 3. Heather als Sender (muss Mitglied sein)
        $heatherId = 1;
        if (!in_array($heatherId, array_map('intval', $memberIds))) {
            logLine("SKIP: Heather ist kein Mitglied von #{$channel['name']}.");
            sleep(5);
            continue;
        }
        $senderId = $heatherId;
        $senderName = 'Heather Mason';

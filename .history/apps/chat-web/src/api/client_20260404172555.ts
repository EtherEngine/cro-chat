...
export type AdminMember = User & { role: string; muted_until: string | null; email: string };
...
export const api = {
  ...
  admin: {
    ...
    moderationLog: (spaceId: number, limit = 50) =>
      request<{ log: ModerationAction[] }>(`/api/spaces/${spaceId}/moderation/log?limit=${limit}`),
  },

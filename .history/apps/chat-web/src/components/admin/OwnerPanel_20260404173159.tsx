    ...
    return (
      <div className="owner-backdrop" onClick={onClose}>
        <div className="owner-modal" onClick={(e) => e.stopPropagation()}>
          <div className="owner-loading">Lade Statistiken...</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="owner-tab-content">
        <div className="owner-stat-grid">
          <StatCard label="Mitglieder" value={stats!.members.total} sub={`${stats!.members.online} online, ${stats!.members.away} abwesend`} color="#7c3aed" />
          <StatCard label="Kanäle" value={stats!.channels.total} sub={`${stats!.channels.public_count} öffentlich, ${stats!.channels.private_count} privat`} color="#2563eb" />
          <StatCard label="Nachrichten" value={stats!.messages.total} sub={`${stats!.messages.today} heute, ${stats!.messages.last_7_days} letzte 7d`} color="#059669" />
          <StatCard label="DMs" value={stats!.conversations.total} color="#d97706" />
          <StatCard label="Dateien" value={stats!.attachments.total} sub={formatBytes(stats!.attachments.total_mb)} color="#dc2626" />
          <StatCard label="Reaktionen" value={stats!.reactions.total} color="#db2777" />
          <StatCard label="Threads" value={stats!.threads.total} color="#0891b2" />
          <StatCard label="Pins" value={stats!.pins.total} color="#65a30d" />
        </div>
        ...
        <div className="owner-role-bars">
          {(['owner', 'admin', 'moderator', 'member', 'guest'] as const).map((r) => {
            const count = stats!.members[r === 'member' ? 'members' : `${r}s` as keyof typeof stats.members] as number;
            if (count === 0) return null;
            const pct = Math.round((count / stats!.members.total) * 100);
            return (
        ...
        <div className="owner-stat-grid owner-stat-grid-small">
          <StatCard label="Aktionen gesamt" value={stats!.moderation.total} color="#6b7280" small />
          <StatCard label="Msg gelöscht" value={stats!.moderation.message_deletes} color="#dc2626" small />
          <StatCard label="Stummschaltungen" value={stats!.moderation.mutes} color="#d97706" small />
          <StatCard label="Kicks" value={stats!.moderation.kicks} color="#7c2d12" small />
          <StatCard label="Rollenänderungen" value={stats!.moderation.role_changes} color="#4f46e5" small />
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="owner-tab-content">
        ...
        <div className="owner-top-list">
          ...
          {stats.topChannels.length === 0 && <p className="owner-empty">Keine Kanäle vorhanden</p>}
        </div>

        <h4 className="owner-section-title">Kanal-Übersicht</h4>
        <div className="owner-stat-grid owner-stat-grid-small">
          <StatCard label="Gesamt" value={stats.channels.total} color="#2563eb" small />
          <StatCard label="Öffentlich" value={stats.channels.public_count} color="#059669" small />
          <StatCard label="Privat" value={stats.channels.private_count} color="#dc2626" small />
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="owner-tab-content">
        ...
        <div className="owner-chart">
          ...
          {stats.dailyMessages.length === 0 && <p className="owner-empty">Keine Daten</p>}
        </div>
        ...
        <div className="owner-stat-grid owner-stat-grid-small">
          <StatCard label="Bearbeitet" value={stats.messages.edited} color="#d97706" small />
          <StatCard label="Gelöscht" value={stats.messages.deleted} color="#dc2626" small />
          <StatCard label="Antworten" value={stats.messages.replies} color="#0891b2" small />
          <StatCard label="Letzte 30d" value={stats.messages.last_30_days} color="#059669" small />
        </div>
      </div>
    );
  }

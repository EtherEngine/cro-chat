...
import { OwnerPanel } from '../admin/OwnerPanel';
...
const BASE_SECTIONS: { key: Section; label: string; icon: string }[] = [
...
export function SettingsModal({ onClose }: Props) {
  ...
  const SECTIONS = isAdminOrOwner
    ? [...BASE_SECTIONS, { key: 'owner' as Section, label: 'Owner Panel', icon: '👑' }]
    : BASE_SECTIONS;
  ...
  function renderContent() {
    switch (active) {
        ...
        return (
          <div className="settings-panel">
            ...
            <label className="settings-toggle">
              <input type="checkbox" checked={settings.notifMentions} onChange={(e) => updateSetting('notifMentions', e.target.checked)} />
              Erwähnungen (@mentions)
            </label>
            <label className="settings-toggle">
              <input type="checkbox" checked={settings.notifDMs} onChange={(e) => updateSetting('notifDMs', e.target.checked)} />
              Direktnachrichten
            </label>
            <label className="settings-toggle">
              <input type="checkbox" checked={settings.notifThreads} onChange={(e) => updateSetting('notifThreads', e.target.checked)} />
              Thread-Antworten
            </label>
            <hr className="settings-divider" />
            <label className="settings-toggle">
              <input type="checkbox" checked={settings.notifSound} onChange={(e) => updateSetting('notifSound', e.target.checked)} />
          ...
          <div className="settings-panel">
            ...
            <label className="settings-label">
              ...
              <select
                ...
                onChange={(e) => updateSetting('fontSize', e.target.value as 'small' | 'normal' | 'large')}
              >
                <option value="small">Klein (13px)</option>
                <option value="normal">Normal (15px)</option>
                <option value="large">Groß (17px)</option>
              </select>
            </label>
            <label className="settings-toggle">
            ...
            <label className="settings-toggle">
              <input type="checkbox" checked={settings.showAvatars} onChange={(e) => updateSetting('showAvatars', e.target.checked)} />
              Avatare anzeigen
            </label>
          </div>
        );
          ...
          <div className="settings-panel">
            ...
            <label className="settings-toggle">
              <input
                ...
                onChange={(e) => updateSetting('showReadReceipts', e.target.checked)}
              />
      ...
      case 'entwickler':
        return (
          <div className="settings-panel">
            ...
            <label className="settings-toggle">
              <input type="checkbox" checked={settings.showQueryStats} onChange={(e) => updateSetting('showQueryStats', e.target.checked)} />
      ...
      case 'owner':
        return <OwnerPanel onClose={onClose} />;
    }
  }

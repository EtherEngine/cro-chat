...
export function SettingsModal({ onClose }: Props) {
  ...
  const [notifThreads, setNotifThreads] = useState(true);
  ...
  const [showQueryStats, setShowQueryStats] = useState(false);
  ...
  async function handleProfileSave() {
    ...
    try {
      await api.users.updateProfile({ display_name: displayName, title });
      dispatch({
        type: 'SET_USER',
        user: { ...state.user, display_name: displayName, title },
      });

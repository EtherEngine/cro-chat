...
final class MessageService
{
    ...
    public static function createChannel(
        ...
        $stop = Metrics::startTimer('message.create');
        ...
        // Check mute state and guest role
        $channelRole = ChannelRepository::memberRole($channelId, $userId);
        $mutedUntil = ChannelRepository::getMutedUntil($channelId, $userId);
        if (!RoleService::canWrite($channelRole ?? 'member', $mutedUntil)) {
            throw ApiException::forbidden('Du bist stummgeschaltet oder hast keine Schreibberechtigung', 'WRITE_DENIED');
        }
        ...
        $result = Database::transaction(function () use ($userId, $body, $channelId, $replyToId, $idempotencyKey) {
    ...
    public static function createConversation(
        ...
        $result = Database::transaction(function () use ($userId, $body, $conversationId, $replyToId, $idempotencyKey) {
            $message = MessageRepository::create(
                $userId,
                $body,
                null,
            ...
            EventRepository::publish('message.created', $room, $message);
            MentionService::processMentions($message['id'], $body, $userId, null, $conversationId, $room);
            NotificationService::notifyDm($message['id'], $userId, $conversationId);
            return $message;
        });
    ...
    public static function update(int $messageId, int $userId, array $input): array
    {
        ...
        MentionService::reprocessMentions(
            $messageId,
            $body,
            (int) $msg['user_id'],
            $updated['channel_id'],
        ...
        // Dispatch search reindex for the edited message
        JobService::dispatch('search.reindex', [
            'action' => 'single',
    ...
    public static function delete(int $messageId, int $userId): void
    {
        ...
        $canDelete = ((int) $msg['user_id'] === $userId)
            ...
            || self::canModerateMessage($msg, $userId);
        ...
        // Dispatch search reindex for the deleted message
        JobService::dispatch('search.reindex', [
            'action' => 'single',
            'message_id' => $messageId,
        ], 'default', 2, 200);
    ...
    /**
     * Returns true if the user can moderate the message's context (moderator+ in channel).
     */
    private static function canModerateMessage(array $msg, int $userId): bool
    {
        if ($msg['channel_id']) {
            $channel = ChannelRepository::find((int) $msg['channel_id']);
            if ($channel) {
                $spaceRole = SpaceRepository::memberRole((int) $channel['space_id'], $userId);
                $channelRole = ChannelRepository::memberRole((int) $msg['channel_id'], $userId);
                return $spaceRole && RoleService::canModerateChannel($spaceRole, $channelRole);
            }
        }
        if ($msg['conversation_id']) {
            $conv = ConversationRepository::find((int) $msg['conversation_id']);
            if ($conv && isset($conv['space_id'])) {
                return SpaceRepository::isModeratorOrAbove((int) $conv['space_id'], $userId);
            }
        }
        return false;
    }

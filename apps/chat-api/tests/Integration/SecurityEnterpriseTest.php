...
final class SecurityEnterpriseTest extends TestCase
{
    ...
    {
        SecretManager::put('vault_test_del', 'data');
        $this->assertEquals('data', SecretManager::get('vault_test_del'));
        SecretManager::delete('vault_test_del');
        $this->assertEquals('', SecretManager::get('vault_test_del'));
    }
    ...
    public function test_vault_list_keys(): void
    {
        ...
        $this->assertContains('a_key', $keyNames);
        $this->assertContains('b_key', $keyNames);
    }
    ...
    public function test_mfa_verify_rejects_invalid_code(): void
    {
        ...
        $this->assertApiException(401, 'MFA_INVALID_CODE', function () use ($user) {
    ...
    public function test_session_mfa_verified(): void
    {
        ...
        // Re-query from DB to confirm update
        $pdo = Database::connection();
        $stmt = $pdo->prepare('SELECT mfa_verified FROM user_sessions WHERE id = ?');
    ...
    public function test_sso_invalid_type_rejected(): void
    {
        ...
        $this->assertApiException(422, 'VALIDATION_ERROR', function () use ($space) {
            SsoService::createProvider($space['id'], [
                'slug' => 'bad',
                'name' => 'Bad',
                'provider_type' => 'ldap',

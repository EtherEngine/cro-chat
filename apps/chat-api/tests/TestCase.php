...
abstract class TestCase extends BaseTestCase
{
    ...
    protected function createAttachment(int $messageId, int $userId, array $overrides = []): array
    {
        $data = array_merge([
            'original_name' => 'test.png',
            'storage_name' => 'att_' . bin2hex(random_bytes(8)) . '.png',
            'mime_type' => 'image/png',
            'file_size' => 1024,
        ], $overrides);
        ...
        $db = Database::connection();
        $db->prepare(
            'INSERT INTO attachments (message_id, original_name, storage_name, mime_type, file_size) VALUES (?, ?, ?, ?, ?)'
        )->execute([$messageId, $data['original_name'], $data['storage_name'], $data['mime_type'], $data['file_size']]);

        return ['id' => (int) $db->lastInsertId(), 'message_id' => $messageId] + $data;
    }

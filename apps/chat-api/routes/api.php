...
// --- Public SSO callback (no auth, handles SSO redirects) ---
$router->post('/api/security/sso/{providerId}/callback', [SecurityController::class, 'ssoCallback']);
$router->post('/api/security/sso/{providerId}/auth-url', [SecurityController::class, 'ssoAuthUrl']);
...
$router->group([AuthMiddleware::class, CsrfMiddleware::class], function ($router) {
    ...
    // Scaling & Infrastructure (admin)
    $router->get('/api/scaling/info', [ScalingController::class, 'info']);
    $router->get('/api/scaling/cache/stats', [ScalingController::class, 'cacheStats']);
    $router->post('/api/scaling/cache/flush', [ScalingController::class, 'cacheFlush']);
    $router->get('/api/scaling/queue/stats', [ScalingController::class, 'queueStats']);
    $router->post('/api/scaling/queue/retry-failed', [ScalingController::class, 'queueRetryFailed']);
    $router->get('/api/scaling/storage/info', [ScalingController::class, 'storageInfo']);
    ...
    // ── Enterprise Security ─────────────────────
    // MFA
    $router->post('/api/security/mfa/setup', [SecurityController::class, 'mfaSetup']);
    $router->post('/api/security/mfa/verify', [SecurityController::class, 'mfaVerify']);
    $router->post('/api/security/mfa/validate', [SecurityController::class, 'mfaValidate']);
    $router->post('/api/security/mfa/disable', [SecurityController::class, 'mfaDisable']);
    $router->post('/api/security/mfa/recovery-codes', [SecurityController::class, 'mfaRecoveryCodes']);
    $router->get('/api/security/mfa/status', [SecurityController::class, 'mfaStatus']);

    // SSO provider management
    $router->get('/api/spaces/{spaceId}/security/sso/providers', [SecurityController::class, 'ssoProviders']);
    $router->post('/api/spaces/{spaceId}/security/sso/providers', [SecurityController::class, 'ssoCreateProvider']);
    $router->put('/api/security/sso/providers/{providerId}', [SecurityController::class, 'ssoUpdateProvider']);
    $router->delete('/api/security/sso/providers/{providerId}', [SecurityController::class, 'ssoDeleteProvider']);
    $router->get('/api/security/sso/links', [SecurityController::class, 'ssoLinks']);
    $router->delete('/api/security/sso/links/{linkId}', [SecurityController::class, 'ssoUnlink']);

    // Device management
    $router->get('/api/security/devices', [SecurityController::class, 'deviceList']);
    $router->post('/api/security/devices/{deviceId}/trust', [SecurityController::class, 'deviceTrust']);
    $router->post('/api/security/devices/{deviceId}/revoke', [SecurityController::class, 'deviceRevoke']);
    $router->delete('/api/security/devices/{deviceId}', [SecurityController::class, 'deviceRemove']);

    // Session management
    $router->get('/api/security/sessions', [SecurityController::class, 'sessionList']);
    $router->delete('/api/security/sessions/{sessionId}', [SecurityController::class, 'sessionRevoke']);
    $router->post('/api/security/sessions/revoke-all', [SecurityController::class, 'sessionRevokeAll']);

    // Security log
    $router->get('/api/security/log', [SecurityController::class, 'securityLog']);

    // ── AI Features ─────────────────────────────────
    // Config
    $router->get('/api/spaces/{spaceId}/ai/config', [AiController::class, 'getConfig']);
    $router->put('/api/spaces/{spaceId}/ai/config', [AiController::class, 'updateConfig']);

    // Summaries
    $router->get('/api/spaces/{spaceId}/ai/summaries', [AiController::class, 'listSummaries']);
    $router->get('/api/ai/summaries/{summaryId}', [AiController::class, 'getSummary']);
    $router->delete('/api/ai/summaries/{summaryId}', [AiController::class, 'deleteSummary']);

    // Sync summarize
    $router->post('/api/threads/{threadId}/ai/summarize', [AiController::class, 'summarizeThread']);
    $router->post('/api/channels/{channelId}/ai/summarize', [AiController::class, 'summarizeChannel']);

    // Async triggers (202 Accepted)
    $router->post('/api/threads/{threadId}/ai/summarize-async', [AiController::class, 'summarizeThreadAsync']);
    $router->post('/api/channels/{channelId}/ai/summarize-async', [AiController::class, 'summarizeChannelAsync']);
    $router->post('/api/spaces/{spaceId}/ai/extract', [AiController::class, 'extract']);
    $router->post('/api/spaces/{spaceId}/ai/embed', [AiController::class, 'embed']);

    // Action Items
    $router->get('/api/spaces/{spaceId}/ai/action-items', [AiController::class, 'listActionItems']);
    $router->put('/api/ai/action-items/{actionItemId}', [AiController::class, 'updateActionItem']);

    // Semantic Search
    $router->get('/api/spaces/{spaceId}/ai/search', [AiController::class, 'search']);

    // Reply Suggestions
    $router->post('/api/spaces/{spaceId}/ai/suggest', [AiController::class, 'suggest']);
    $router->post('/api/ai/suggestions/{suggestionId}/accept', [AiController::class, 'acceptSuggestion']);

    // Stats
    $router->get('/api/spaces/{spaceId}/ai/stats', [AiController::class, 'stats']);
});

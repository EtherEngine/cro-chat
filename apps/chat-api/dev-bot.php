...
/**
 * Developer Bot – Heather schreibt DMs an zufällige Co-Mitglieder.
 *
 * Every ~5 minutes heather.mason@cro.dev sends a random co-member a DM.
...
$recipients = array_values(array_filter($allMembers, fn($m) => (int) $m['id'] !== $heatherId));
if (empty($recipients)) {
    logLine("FATAL: Keine anderen Mitglieder im Space.");
    exit(1);
}
logLine("Empfänger-Pool: " . count($recipients) . " User");
...
while (!$shouldStop) {
    ...
    // Reconnect DB before each iteration (PDO can go stale after long sleep)
    try {
    ...
    try {
        ...
        // 3. Simulate auth as Heather
        $_SESSION['user_id'] = $heatherId;

        // 4. Get-or-create 1:1 DM conversation
        $conv = ConversationService::getOrCreateDirect($spaceId, $heatherId, $recipientId);
        $convId = (int) $conv['id'];

        // 5. Send message as Heather
        $message = MessageService::createConversation($convId, $heatherId, [
            'body' => $body,
        ]);

        logLine(sprintf(
            "OK  sender=%d (%s)  target=%d (%s)  conv=%d  msg=%d  body=\"%s\"",
            $heatherId,
            $heather['display_name'],
            $recipientId,

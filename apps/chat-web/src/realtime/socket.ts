...
export class RealtimeClient {
  ...
  /** Track last event ID for delta sync on reconnect. */
  ...
  private openSocket(ticket: string): void {
    ...
    this.ws.onopen = () => {
      ...
      const wasReconnect = this.connectedSince !== null;
      this.connected = true;
  ...
  /**
   * Get the last known event ID (for delta sync).
   */
  ...
  /**
   * Get connection uptime in ms (or null if disconnected).
   */
  getUptime(): number | null {
    return this.connectedSince ? Date.now() - this.connectedSince : null;
  }

  /**
   * Force an immediate reconnect (e.g., after coming back online).
   */
  forceReconnect(): void {
    if (this.disposed) return;
    if (this.reconnectTimer) {
